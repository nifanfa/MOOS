...
static char *GetGameName(char *gamename)
{
    ...
    for (i=0; i<arrlen(banners); ++i)
    {
        ...
        if (deh_sub != banners[i])
        {
            ...
            gamename_size = strlen_(deh_sub) + 10;
            gamename = Z_Malloc(gamename_size, PU_STATIC, 0);
            version = G_VanillaVersionCode();
            while (gamename[0] != '\0' && isspace_((int)gamename[0]))
            {
                memmove_(gamename, gamename + 1, gamename_size - 1);
            }

            while (gamename[0] != '\0' && isspace_((int)gamename[strlen_(gamename)-1]))
            {
                gamename[strlen_(gamename) - 1] = '\0';
            }
...
static void InitGameVersion(void)
{
    ...
    if (p)
    {
        ...
        {
            if (!strcmp_(myargv[p+1], gameversions[i].cmdline))
            {
...
static void LoadIwadDeh(void)
{
    ...
    if (gameversion == exe_chex)
    {
        ...
        sep = mystrrchr(iwadfile, DIR_SEPARATOR);

        if (sep != NULL)
        {
            size_t chex_deh_len = mystrlen(iwadfile) + 9;
            chex_deh = kmalloc(chex_deh_len);
...
//    W_PrintDirectory();
    ...
    if (M_ParmExists("-dehlump"))
    {
        ...
        {
            if (!mystrncmp(lumpinfo[i].name, "DEHACKED", 8))
            {
