...
static void CheckUninstallStrings(void)
{
    ...
    for (i=0; i<arrlen(uninstall_values); ++i)
    {
        ...
        unstr = strstr_(val, UNINSTALLER_STRING);
        ...
        {
            path = unstr + strlen_(UNINSTALLER_STRING);
...
static void CheckSteamGUSPatches(void)
{
    ...
    if (current_path != NULL && strlen_(current_path) > 0)
    {
        return;
    }
    ...
    len = strlen_(install_path) + strlen_(STEAM_BFG_GUS_PATCHES) + 20;
...
static boolean DirIsFile(char *path, char *filename)
{
    ...
    path_len = strlen_(path);
    filename_len = strlen_(filename);
