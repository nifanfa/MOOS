...
unsigned int W_LumpNameHash(const char *s)
{
    ...
    for (i=0; i < 8 && s[i] != '\0'; ++i)
    {
        result = ((result << 5) ^ result ) ^ toupper_((int)s[i]);
    }
...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    newlumpinfo = kcalloc(newnumlumps, sizeof(lumpinfo_t));
...
wad_file_t *W_AddFile (char *filename)
{
    ...
    if (strcasecmp(filename+strlen_(filename)-3 , "wad" ) )
    {
    	// single lump file
    ...
    else 
    {
		...
		if (strncmp_(header.identification,"IWAD",4))
		{
			// Homebrew levels?
			if (strncmp_(header.identification,"PWAD",4))
			{
    ...
    for (i=startlump; i<numlumps; ++i)
    {
		...
		lump_p->size = LONG(filerover->size);
			lump_p->cache = NULL;
		strncpy_(lump_p->name, filerover->name, 8);
