...
namespace MOOS.Driver
{
	public static unsafe class PCIExpress
	{
		...
		public static void Initialize()
		{
			...
			long numEntries = (ACPI.MCFG->Header.Length - sizeof(ACPI.MCFGHeader) + sizeof(ACPI.MCFGEntry)) / sizeof(ACPI.MCFGEntry);

			for (int i = 0; i < numEntries; i++)
			{
				ACPI.MCFGEntry* Entries = &ACPI.MCFG->Entry0;
				for (byte bus = Entries->StartBus; bus < Entries->EndBus; bus++)
				{
		...
		public static void CheckBus(ulong BaseAddress, byte Bus, ushort Segment)
		{
			...
			if (Header0->DeviceID == 0 || Header0->DeviceID == 0xFFFF)
			{
				return;
			}
			...
			for (byte Slot = 0; Slot < 32; Slot++)
			{
				...
				if (Header1->DeviceID == 0 || Header1->DeviceID == 0xFFFF)
				{
					return;
				}
				...
				for (byte Func = 0; Func < 8; Func++)
				{
					...
					if (Dev->Header.DeviceID == 0 || Dev->Header.DeviceID == 0xFFFF)
					{
						return;
					}
					...
					PCIDevice device = new()
					{
						...
						Slot = Slot,
						Function = Func,
						VendorID = Dev->Header.VendorID,
						IsPCIEDevice = true,

						ClassID = Dev->Header.ClassID,
						SubClassID = Dev->Header.SubClass,
						ProgIF = Dev->Header.ProgIF,

						DeviceID = Dev->Header.DeviceID
					};
					...
					Console.WriteLineInfo("PCI Express", $"{device.Bus}:{device.Slot}:{device.Function}] {VendorID.GetName(device.VendorID)} {ClassID.GetName(device.ClassID)}");
