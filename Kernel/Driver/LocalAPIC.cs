...
namespace MOOS.Driver
{
    public static unsafe partial class LocalAPIC
    {
        ...
        public static void Initialize()
        {
            ...
            WriteRegister(LAPIC_SVR, 0x1FF);

            if (SMP.ThisCPU == 0)
            {
                Console.WriteLineInfo("Local APIC", "Local APIC initialized");
            }
        }

        public static uint GetId()
        {
            return ReadRegister(LAPIC_ID) >> 24;
        }
        ...
        {
            WriteRegister(LAPIC_ICRHI, apic_id << ICR_DESTINATION_SHIFT);
            WriteRegister(LAPIC_ICRLO, vector);

            while ((ReadRegister(LAPIC_ICRLO) & ICR_SEND_PENDING) != 0)
            {
                ;
            }
        }
