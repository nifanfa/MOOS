...
{
    public static unsafe class IDE
    {
        ...
        public static void Initialize()
        {
            ...
            if (Ports.Count != 0)
            {
                Console.WriteLineInfo("IDE", "IDE controller Initialized");
            }
        }
        ...
        public static void ScanPorts(Channels index)
        {
            ...
            for (byte port = 0; port < 2; port++)
            {
                ...
                fixed (byte* p = DriveInfo)
                {
                    ...
                    Console.WriteInfo("IDE", " ");
                    byte* pName = p + IDEDevice.ModelNumber;
                    for (int i = 0; i < 40; i++)
                    {
                ...
                IDEDevice drv = new()
                {
                    LBALowPort = LBALowPort,
                    LBAMidPort = LBAMidPort,
                    LBAHighPort = LBAHighPort,
                    DeviceHeadPort = DeviceHeadPort,
                    StatusPort = StatusPort,
                    CommandPort = CommandPort,


                    DataPort = DataPort,
                    SectorCountPort = SectorCountPort,
