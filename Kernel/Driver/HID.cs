    ...
    public static unsafe class HID
    {
        ...
        public static void Initialize(USBDevice device)
        {
            if (device.Protocol == 1)
            {
                ...
                Console.WriteLineInfo("HID", $"USB Keyboard at port:{device.Address}");
                InitializeKeyboard(device);
            } else if (device.Protocol == 2)
            {
                USB.NumDevice++;

                Console.WriteLineInfo("HID", $"USB Mouse at port:{device.Address}");
        ...
        private static void InitializeMouse(USBDevice device)
        {
            Mouse = device;
        }

        private static void InitializeKeyboard(USBDevice device)
        {
            Keyboard = device;
