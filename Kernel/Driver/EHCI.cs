...
namespace MOOS.Driver
{
    public static unsafe class EHCI
    {
        ...
        public static void Initialize()
        {
            ...
            Console.WriteLineInfo("EHCI", "EHCI controller found!");

            device.WriteRegister(0x04, 0x04 | 0x02 | 0x01);

            uint bar0 = device.Bar0;
            Console.WriteLineInfo("EHCI", $"Bar0: {bar0.ToString("x2")}");
            ...
            Console.WriteLineInfo("EHCI", $"{AvailablePorts} Ports available");
            uint hccparams = *(uint*)(bar0 + 0x08);

            uint eecp = (hccparams & (255 << 8)) >> 8;
            if (eecp >= 0x40)
            {
                Console.WriteLineInfo("EHCI", "Disabling BIOS EHCI Hand-off");
                ...
                if (legsup & 0x00010000)
                {
                    ...
                    for (; ; )
                    {
                        Console.WriteLineInfo("EHCI", "Waitting for BIOS ready");
            ...
            if (default_cmd & 1)
            {
                Console.WriteLineInfo("EHCI", "Stopping this controller");
                *(uint*)CMDReg &= ~1u;
                while (1)
                {
                    if ((*(uint*)CMDReg & 1) == 0)
            ...
            *(uint*)CMDReg |= 2;
            while (1)
            {
                Console.WriteLineInfo("EHCI", "Waiting for controller ready");
                if ((*(uint*)CMDReg & 2) == 0)
                {
                    break;
                }
            ...
            *(uint*)(BaseAddr + 0x40) |= 1;
            ...
            Console.WriteLineInfo("EHCI", "EHCI controller initialized");
        }
        ...
        private static QH* qh;
        private static QH* qh1;
        private static QH* qh2;
        private static TD* td;
        private static TD* trans;
        private static TD* trans1;
        private static TD* sts;
        private static USBRequest* cmd;
        ...
        private static byte SetDeviceAddr(byte addr, USBDevice parent, int speed)
        {
            (*cmd).Clean();
            (*trans).Clean();
            (*sts).Clean();
        ...
        private static byte WaitForComplete(TD* status)
        {
            Native.Hlt();
            ...
            if (lsts == 0)
            {
                //Console.WriteLineInfo("EHCI", "Transmission failed");
            }
            return lsts;
        }

        private static byte* GetDesc(byte addr, byte size, USBDevice parent, int speed)
        {
            (*qh).Clean();
            (*qh1).Clean();
            (*qh2).Clean();
            ...
            *(uint*)CMDReg &= ~0x20u;
            *(uint*)AsyncListReg = 1;
            return result == 0 ? (byte*)null : buffer;
        }

        private static byte* GetConfig(byte addr, byte size, USBDevice parent, int speed)
        {
            (*qh).Clean();
            (*qh1).Clean();
            (*qh2).Clean();
            ...
            *(uint*)CMDReg &= ~0x20u;
            *(uint*)AsyncListReg = 1;
            return res == 0 ? (byte*)null : buffer;
        }

        private static byte SetConfig(byte addr, byte config, USBDevice parent, int speed)
        {
            (*cmd).Clean();
            (*td).Clean();
            (*sts).Clean();
        ...
        public static bool InitPort(int port, USBDevice parent, int speed)
        {
            ...
            USB.DeviceAddr++;
            Console.WriteLineInfo("EHCI", $"Next device address is {USB.DeviceAddr}");
            ...
            if (parent == null)
            {
                ...
                *(uint*)reg_port &= ~0x100u;
                ...
                if ((portinfo & 4) == 0)
                {
                    Console.WriteLineInfo("EHCI", $"Port {port} Is not enabled");
            ...
            byte addr = SetDeviceAddr(USB.DeviceAddr, parent, speed);
            if (addr == 0)
            {
                Console.WriteLineInfo("EHCI", $"Port {port} Failed to set device address");
            ...
            byte* _desc = GetDesc(USB.DeviceAddr, 8, parent, speed);
            if (_desc == 0)
            {
                Console.WriteLineInfo("EHCI", $"Port {port} Failed to get descriptor");
            ...
            {
                Console.WriteLineInfo("EHCI", $"Port {port} Invalid magic number");

                device.Dispose();
                return false;
            }
            byte max_packet_size = _desc[7];
            Console.WriteLineInfo("EHCI", $"Port {port} Max Packet Size {max_packet_size}");

            ConfigDesc* cdesc = (ConfigDesc*)GetConfig(USB.DeviceAddr, (byte)(sizeof(InterfaceDesc) + sizeof(ConfigDesc) + (sizeof(EndPoint) * 2)), parent, speed);
            if (cdesc == 0)
            {
                Console.WriteLineInfo("EHCI", $"Port {port} Failed to get descriptor");
            ...
            Console.WriteLineInfo("EHCI", $"Port {port} Class: {Class}");

            byte config_res = SetConfig(USB.DeviceAddr, 1, parent, speed);
            if (config_res == 0)
            {
                Console.WriteLineInfo("EHCI", $"Port {port} failed to set configuration");
        ...
        public static void ScanPorts()
        {
            ...
            for (int i = 0; i < AvailablePorts; i++)
            {
                ...
                Console.WriteLineInfo("EHCI", $"Port {i} {((*(uint*)reg_port & 3) ? "Present" : "Not present")}");
                if (*(uint*)reg_port & 3)
                {
