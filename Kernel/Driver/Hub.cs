...
namespace MOOS.Driver
{
	public static unsafe class Hub
	{
		...
		internal static void Initialize(USBDevice device)
		{
			...
			if (!b)
			{
				Console.WriteLineInfo("USB Hub", "Can't get Hub descriptor");
			}

			Console.WriteLineInfo("USB Hub", $"This hub has {desc->PortCount} ports");
			...
			for (int i = 0; i < desc->PortCount; i++)
			{
				...
				for (int k = 0; k < 8; k++)
				{
					...
					if (!b)
					{
						Console.WriteLineInfo("USB Hub", $"Can't get Hub port {i} status");
						return;
					}
