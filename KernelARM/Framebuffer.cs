...
namespace Kernel
{
    public static unsafe class Framebuffer
    {
        ...
        {
            Stosd(TripleBuffered ? FirstBuffer : VideoMemory, Color, (ulong)(Width * Height));
        }
        ...
        public static void Update()
        {
            if (TripleBuffered)
            {
                ...
                Movsd(SecondBuffer, FirstBuffer, (ulong)(Width * Height));
            }
        }
        ...
        public static void SetVideoMode(uint XRes, uint YRes)
        {
            ...
            Stosd(FirstBuffer, 0, (ulong)(XRes * YRes));
            Stosd(SecondBuffer, 0, (ulong)(XRes * YRes));
            Control.MousePosition.X = (int)(XRes / 2);
            Control.MousePosition.Y = (int)(YRes / 2);
            _TripleBuffered = false;
        }

        public static void Stosd(uint* dest, uint value, ulong count)
        {
            for (ulong u = 0; u < count; u++) dest[u] = value;
        }

        public static void Movsd(uint* dest, uint* source, ulong count)
        {
            for (ulong u = 0; u < count; u++) dest[u] = source[u];
        }
