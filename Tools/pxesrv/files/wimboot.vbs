...
For Each aItem In pCurrentDir.Files
   ...
   If LCase(Right(Cstr(aItem.Name), 3)) = "wim" Then
    c=c+1
    if s=1 then wscript.echo "item " & c & chr(9) & replace(replace(aItem.path,strweb,""),"\","/") 
	dim path
	...
	if s=2 then 
		wscript.echo ":" & c 
		wscript.echo "kernel ${boot-url}/wimboot"
		wscript.echo "iseq ${platform} pcbios && initrd -n bcd ${boot-url}/BOOT/BCD bcd ||"		
		wscript.echo "iseq ${platform} efi    && initrd -n bcd ${boot-url}/EFI/MICROSOFT/BOOT/BCD bcd ||"
		wscript.echo "initrd -n boot.sdi ${boot-url}/BOOT/BOOT.SDI boot.sdi"
		wscript.echo "initrd -n boot.wim ${boot-url}" & path & " boot.wim"
		wscript.echo "boot"
	end if
   End If
Next
