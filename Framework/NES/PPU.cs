...
namespace NES
{
	public class PPU
	{
	 ...
	 {0x04, 0x04, 0x06, 0x06}};
		...
		// Sprite Variables
		private int spScanlineNumber = 0;
	...
	{0xFF, 0xE7, 0xA3, 0x00}, {0xE3, 0xFF, 0xA3, 0x00}, {0xAB, 0xF3, 0xBF, 0x00}, {0xB3, 0xFF, 0xCF, 0x00}, {0x9F, 0xFF, 0xF3, 0x00}, {0x00, 0x00, 0x00, 0x00}, {0x00, 0x00, 0x00, 0x00}, {0x00, 0x00, 0x00, 0x00}};
		...
		public void renderPixel()
		{
			...
			bolDrawBG = (memory.memCPU[0x2001] & 0x08) == 0x08;
			...
			// Check to see if BG is turned on
			bolDrawSprites = (memory.memCPU[0x2001] & 0x10) == 0x10;
		...
		private void UpdateDrawLocation()
		{
			...
			} else if (pixelM % 256 == 0)
			{
				...
				if ((v & 0x7000) == 0x7000)
				{
					...
					if ((v & 0x03E0) == 0x3A0)
					{ v ^= 0x800 | 0x3A0; } else if ((v & 0x03E0) == 0x3E0)
					{ v ^= 0x3E0; } else
					{ v += 0x0020; }
				} else
				{
