...
namespace MOOS
{
	...
	public static unsafe class PCI
	{
		...
		public static void Initialize()
		{
			...
			Console.WriteInfo("PCI", "PCI Initialized.");
		...
		public static void CheckBus(ushort Bus)
		{
			for (ushort slot = 0; slot < 32; slot++)
			{
				...
				Console.WriteLineInfo("PCI", $"[{device.Bus}:{device.Slot}:{device.Function}] {VendorID.GetName(device.VendorID)} {ClassID.GetName(device.ClassID)}");
