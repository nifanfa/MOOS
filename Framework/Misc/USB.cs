...
namespace MOOS.Misc
{
	...
	public static unsafe class USB
	{
		...
		{
			return device.USBVersion == 2 && EHCI.SendAndReceive(device.Address, cmd, buffer, parent);
		}
		...
		{
			return version == 2 && EHCI.InitPort(port, parent);
		}
		...
		public static void DriveDevice(USBDevice device)
		{
			switch (device.Class)
			{
				...
				case 9:
					...
					break;
				default:
					Console.WriteLineInfo("USB", $"Unrecognized device class:{device.Class} subClass:{device.SubClass}");
					break;
